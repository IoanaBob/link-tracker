class TrackingLinksController < ApplicationController
  def create
    ...
    if tracking_link.save
      tracking_link_path = "#{request.base_url}#{tracked_action_persist_path(message_id: params[:message_id])}"
      json_response(data: { tracking_link: tracking_link_path } )
    else
      json_response(data: { error: "Couldn't create tracking link. Please try again." } )
    end
  end
